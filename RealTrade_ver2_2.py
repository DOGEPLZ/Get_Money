...
while True:
    try:
        ...
        get_target_price(ticker) > 0:
            cash = upbit.get_balance()
            target_price = get_target_price_2(ticker)
            current_price = get_current_price(ticker)
            buy_price = round(current_price * 1.0001, -3)
            ...
            if target_price <= current_price and target_price > 0:
                ...
                print('매수주문', 'cash:', cash, current_price, False, True)
                wait_flag = True
                time.sleep(0.1)

        if hold_flag == False and wait_flag == True:
            uncomp = upbit.get_order(ticker)
            changed_price = get_target_price_2(ticker)
            if len(uncomp) == 0:
        ...
        if hold_flag == True and wait_flag == True:
            ...
            if len(uncomp) == 0:
                cash = upbit.get_balance()
                print("매도완료", "cash:", cash, False, False)
                print("        ", '수익률:', sell_price/buy_price)
                hold_flag = False
                wait_flag = False
                print("거래 대기")
                A = 0
                while target_price == get_target_price_2(ticker):
                    A = A + 1
                A = 0 
                print("거래 재개")
                ...
                if len(uncomp) == 0:
                    ...
                    print("        ",  '수익률:', get_current_price(ticker)/buy_price)
                    print('거래 중지')
                    A = 0
                    while target_price == get_target_price_2(ticker):
                        A = A + 1
